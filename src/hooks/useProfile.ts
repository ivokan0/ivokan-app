...
export const useProfile = () => {
  ...
  const updateUserProfile = useCallback(async (
    updates: Partial<Pick<Profile, 'first_name' | 'last_name' | 'avatar_url' | 'timezone' | 'minimum_time_notice' | 'break_duration_minutes' | 'biography' | 'country_birth' | 'presentation_video_url'>>
  ) => {
    if (!user) {
